...
The average time for each algorithm (in seconds) was recorded and can be found in Table 1, and  
Figure 6 is a graph of these averages. We wanted to see how consistent the algorithms were behaving, and figures 2 through 5 show each trial's 
execution time (for each algorithm). Quicksort ran incredibly fast, so we decided to run it on larger 
lists (up to n = 100,000,000). Table 2 shows the Quicksort against the calculated $n$ log $n$ values.
